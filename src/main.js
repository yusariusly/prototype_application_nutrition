...
// ==================== CHART ZOOM MODAL CONTROLLERS ====================
let zoomedChartInstance = null;

window.openChartZoomModal = function(type) {
    state.zoomedChartType = type;
    ...
    const rangeSelect = document.getElementById('zoom-modal-range');
    
    if (type === 'weight') {
        titleEl.innerText = 'Zoomed Weight Trend (lbs)';
        const mainRange = document.getElementById('weight-trend-range')?.value || '3m';
        if (rangeSelect) rangeSelect.value = mainRange;
    } else {
        titleEl.innerText = 'Zoomed Body Measurements (Inches)';
        const mainRange = document.getElementById('measurements-range')?.value || '3m';
        if (rangeSelect) rangeSelect.value = mainRange;
    }
...
window.closeChartZoomModal = function() {
    const modal = document.getElementById('chart-zoom-modal');
    if (modal) {
        modal.classList.add('hidden');
        modal.classList.remove('flex');
...
window.handleZoomModalRangeChange = function() {
    const rangeSelect = document.getElementById('zoom-modal-range');
    if (!rangeSelect) return;
    const val = rangeSelect.value;
    
    if (state.zoomedChartType === 'weight') {
        const mainSelect = document.getElementById('weight-trend-range');
        if (mainSelect) {
            mainSelect.value = val;
            updateWeightTrendChart();
        }
    } else {
        const mainSelect = document.getElementById('measurements-range');
        if (mainSelect) {
            mainSelect.value = val;
            updateMeasurementsChart();
        }
...
window.updateZoomedChart = function() {
    const canvas = document.getElementById('zoomedChartCanvas');
    if (!canvas) return;
    const ctx = canvas.getContext('2d');
    
    const type = state.zoomedChartType;
    const range = document.getElementById('zoom-modal-range')?.value || '3m';
    ...
    if (type === 'weight') {
        ...
        let filtered = [];
        if (range === '1w') filtered = sorted.slice(-7);
        else if (range === '1m') filtered = sorted.slice(-30);
        else filtered = sorted.slice(-90);
        ...
        const numPoints = weights.length;
        const pointRadius = numPoints > 30 ? 2 : 5; 
        const maxTicks = numPoints > 30 ? 12 : 7;
        ...
        zoomedChartInstance = new Chart(ctx, {
            ...
            options: {
                ...
                plugins: { legend: { display: false } },
                scales: {
                    y: { grid: { color: 'rgba(211, 228, 254, 0.4)' }, ticks: { color: '#6d7b6c', font: { size: 11 } } },
                    x: { grid: { display: false }, ticks: { color: '#6d7b6c', font: { size: 11, weight: '500' }, maxTicksLimit: maxTicks } }
                }
            }
        });
    } else {
        ...
        zoomedChartInstance = new Chart(ctx, {
            ...
            options: {
                ...
                maintainAspectRatio: false,
                plugins: {
                    legend: { position: 'bottom', labels: { color: '#0b1c30', font: { size: 11, weight: '600' } } }
                },
                scales: {
                    y: { grid: { color: 'rgba(211, 228, 254, 0.4)' }, ticks: { color: '#6d7b6c', font: { size: 11 } } },
                    x: { grid: { display: false }, ticks: { color: '#6d7b6c', font: { size: 11, weight: '500' }, maxTicksLimit: maxTicks } }
