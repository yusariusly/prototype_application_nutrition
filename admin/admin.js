...
window.renderSpecialistReports = function() {
    const activeSpecialist = localStorage.getItem('nutriflow_specialist_name') || 'Dr. Hasan';
    const myClients = state.clients.filter(c => c.therapist === activeSpecialist);
    
    // Stats elements
    const myClientsCountEl = document.getElementById('rep-my-clients-count');
    const myComplianceEl = document.getElementById('rep-my-compliance');
    const myWaterEl = document.getElementById('rep-my-water');
    const myPendingSessionsEl = document.getElementById('rep-my-pending-sessions');
    ...
    if (clientCount > 0) {
        const sumComp = myClients.reduce((acc, c) => acc + (c.compliance || 85), 0);
        avgComp = Math.round(sumComp / clientCount);
        
        myClients.forEach(c => {
            const clientWater = parseInt(localStorage.getItem('nutriflow_water_glasses_' + c.name)) || 5;
            totalWater += clientWater;
        });
        totalWater = (totalWater / clientCount).toFixed(1);
    }
    
    const pendingSessions = state.appointments.filter(a => a.therapist === activeSpecialist && a.status === 'pending').length;
    
    if (myClientsCountEl) myClientsCountEl.innerText = clientCount;
    if (myComplianceEl) myComplianceEl.innerText = `${avgComp}%`;
    if (myWaterEl) myWaterEl.innerText = `${totalWater} gls`;
    if (myPendingSessionsEl) myPendingSessionsEl.innerText = pendingSessions;
    
    const tbody = document.getElementById('specialist-reports-table-body');
    if (!tbody) return;
    
    if (clientCount === 0) {
        tbody.innerHTML = `
            <tr>
                <td colspan="6" class="p-8 text-center text-xs text-on-surface-variant font-medium">
                    No clients assigned to you. Go to Super Admin to allocate clients.
                </td>
            </tr>
        `;
        return;
    ...
    tbody.innerHTML = myClients.map(c => {
        const clientProgram = state.programs.find(p => p.id === c.activeProgramId) || { name: 'No Active Program' };
        
        // Eaten calories computation for today (Wed)
        const today = 'Wed';
        const clientMealPlans = JSON.parse(localStorage.getItem('nutriflow_client_meal_plans')) || {};
        const clientPlan = clientMealPlans[c.name]?.[today] || [];
        
        const loggedStatus = JSON.parse(localStorage.getItem('nutriflow_logged_status')) || {};
        const clientLogged = loggedStatus[c.name]?.[today] || {};
        
        let eaten = 0;
        let total = 0;
        clientPlan.forEach(meal => {
            if (clientLogged[meal.type] === true) {
                eaten += meal.calories;
            }
            total += meal.calories;
        });
        
        const targetKcal = parseInt(localStorage.getItem('nutriflow_target_kcal_' + c.name)) || 2100;
        const water = parseInt(localStorage.getItem('nutriflow_water_glasses_' + c.name)) || 5;
        const compliance = c.compliance || 85;
        
        let complianceColor = 'text-green-600';
        if (compliance < 80) complianceColor = 'text-amber-600';
        if (compliance < 70) complianceColor = 'text-red-600';
        
        const initials = c.name.split(' ').map(s => s[0]).join('').substring(0, 2).toUpperCase();
        
        return `
            <tr class="hover:bg-surface-container-lowest/40 transition-colors">
                <td class="p-4 pl-6 flex items-center gap-3">
                    ...
                    <div>
                        <span class="font-bold text-on-background block leading-tight">${c.name}</span>
                        <span class="text-[9px] text-on-surface-variant/80 block mt-0.5">${c.email}</span>
                    </div>
                </td>
                <td class="p-4 font-semibold text-on-background">${clientProgram.name}</td>
                <td class="p-4 text-center font-bold text-slate-700">${eaten} / ${targetKcal} kcal</td>
                <td class="p-4 text-center font-bold text-blue-600">${water} / 8 glasses</td>
                <td class="p-4"><span class="font-bold ${complianceColor}">${compliance}%</span></td>
                <td class="p-4 pr-6 text-right">
                    <button onclick="viewClientMealLogs('${c.name}')" class="bg-primary hover:bg-[#005321] text-white font-bold text-[10px] px-3 py-1.5 rounded-lg shadow-sm transition-all active:scale-95 cursor-pointer inline-flex items-center gap-1">
                        <span class="material-symbols-outlined text-[12px]">menu_book</span> View Logs
                    </button>
                </td>
            </tr>
        `;
...
window.viewClientMealLogs = function(clientName) {
    const today = 'Wed';
    const clientMealPlans = JSON.parse(localStorage.getItem('nutriflow_client_meal_plans')) || {};
    const clientPlan = clientMealPlans[clientName]?.[today] || [];
    
    const loggedStatus = JSON.parse(localStorage.getItem('nutriflow_logged_status')) || {};
    const clientLogged = loggedStatus[clientName]?.[today] || {};
    
    const modal = document.getElementById('client-logs-modal');
    ...
    title.innerText = `${clientName}'s Daily Log`;
    subtitle.innerText = `Tracked meals and macros for today (Wednesday)`;
    
    if (clientPlan.length === 0) {
        content.innerHTML = `<div class="text-xs text-on-surface-variant font-medium text-center py-6">No meals planned for today in client's program.</div>`;
    } else {
        content.innerHTML = clientPlan.map(meal => {
            const isLogged = clientLogged[meal.type] === true;
            const statusText = isLogged ? 'Eaten & Tracked' : 'Not logged yet';
            const statusBg = isLogged ? 'bg-green-100 text-green-800 border-green-300' : 'bg-slate-100 text-slate-500 border-slate-200';
            const checkIcon = isLogged ? 'check_circle' : 'pending';
            
            return `
                <div class="flex items-center justify-between p-3 border rounded-xl bg-slate-50">
                    <div class="min-w-0">
                        <div class="flex items-center gap-2">
                            <span class="font-bold text-slate-800 text-xs">${meal.type}</span>
                            <span class="text-[10px] text-slate-400 font-semibold">${meal.calories} kcal</span>
                        </div>
                        <p class="text-[10px] text-slate-600 truncate mt-0.5">${meal.title}</p>
                    </div>
                    <span class="border px-2.5 py-1 rounded-full text-[9px] font-bold flex items-center gap-1 ${statusBg}">
                        <span class="material-symbols-outlined text-[10px]">${checkIcon}</span> ${statusText}
                    </span>
                </div>
            `;
        }).join('');
...
window.closeClientLogsModal = function() {
    const modal = document.getElementById('client-logs-modal');
    if (modal) {
        modal.classList.add('hidden');
        modal.classList.remove('flex');
